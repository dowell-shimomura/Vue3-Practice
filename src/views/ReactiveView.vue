<template>
  <h1>Reactive Programming in Vue 3</h1>
  <div>
    <h2>ref()</h2>
    <p>カウント: {{ count }}</p>
  ...
  <div>
    <h2>v-bind</h2>
    <p>
      Vueの公式サイト: <a :href="vueURL">{{ vueURL }}</a>
    </p>
  </div>
  ...
  <div>
    ...
    <p>
      BMI:<span v-if="bmi !== null">{{ bmi }}</span>
    </p>
  </div>
  ...
  <div>
    ...
    <p v-if="bmiCategory" style="color: blue">肥満度: {{ bmiCategory }}</p>
  </div>
</template>
...
// v-htmlの例
const htmlcontent = ref('<h4>Password</h4><input type="password" />を表示します。')
